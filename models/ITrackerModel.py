...
def itracker_net(train=True):
    ...
    # Data layer
    if train:
        n.data, n.label = L.Data(source="train_data.txt", backend=P.Data.LMDB, batch_size=64, ntop=2)
    else:
        n.data = L.Data(source="test_data.txt", backend=P.Data.LMDB, batch_size=64, ntop=1)
    ...
    n.fc1 = L.InnerProduct(n.pool2, num_output=256, weight_filler=dict(type='xavier'))
    n.relu3 = L.ReLU(n.fc1, in_place=True)
    
    # Output layer
    n.score = L.InnerProduct(n.relu3, num_output=NUM_CLASSES, weight_filler=dict(type='xavier'))
...
with open('itracker_test.prototxt', 'w') as f:
    f.write(str(itracker_net(train=False)))
